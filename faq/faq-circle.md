...
具体使用可以参考[圈选-定义页面](faq-circle.md)。
...
### 1.6 **今天创建的指标，我点进去选过去 7 天和过去 14 天，为什么数据是一样的？**

如果是新创建的指标，指标需要回溯，而且只会回溯创建过去 7 天的数据，因此过去 14 天到过去 7 天都没数。

## 2. web圈选  <a id="2"></a>

**2.1 为什么 web 圈选的元素只有点击量，没有浏览量？**
...
**2.3 如何对带 hashtag 的url进行各页面浏览量统计？**
...
**2.5 web 圈选时候页面加载不完全？错位？**

部分浏览器存在不兼容的情况，推荐使用 Chrome 浏览器。

**2.6 web 端已经装了 SDK，但是仍不能进行圈选。**
